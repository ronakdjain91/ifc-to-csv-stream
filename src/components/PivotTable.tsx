...
export const PivotTable = ({ elements, selectedElement, onElementSelect }: PivotTableProps) => {
  ...
  const pivotData = useMemo(() => {
    ...
    elements.forEach(element => {
      const rowKey = element[rowField as keyof IFCElement] as string || 'Unknown';
      const colKey = element[columnField as keyof IFCElement] as string || 'Unknown';
      ...
      data[rowKey].children![colKey].elements.push(element);
      data[rowKey].children![colKey].count++;
      data[rowKey].children![colKey].totalArea = (data[rowKey].children![colKey].totalArea || 0) + (element.properties?.area || 0);
      data[rowKey].children![colKey].totalVolume = (data[rowKey].children![colKey].totalVolume || 0) + (element.properties?.volume || 0);
    });
    ...
    Object.entries(pivotData).forEach(([rowKey, rowData]) => {
      rowData.elements.forEach(element => {
        exportData.push({
          ...
          'Type': element.type,
          'Level': element.level,
          'Material': element.material?.name || 'N/A',
